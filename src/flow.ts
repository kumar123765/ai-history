...
import { parseISODateUTC, monthsFull, jaccard, stripKnownPrefixAll, trimSummary, norm } from "./helpers/utils.js";
import { wikiOnThisDay, extractWikiList, requireDateConsensus, wikiSummaryByTitle } from "./helpers/wiki.js";
...
type S = {
  ...
  mm?: string; dd?: string; readableDate?: string;
  wiki?: EventItem[];
  px?: PXItem[];
  merged?: EventItem[];
  selected?: EventItem[];
...
function pickWithBounds(events: EventItem[], total: number, indianMin: number, indianMax: number) {
  const indianAll = events.filter(e => e.is_indian).sort((a,b)=> (b.score||0)-(a.score||0));
  const globalAll = events.filter(e => !e.is_indian).sort((a,b)=> (b.score||0)-(a.score||0));
  let indianTarget = Math.max(indianMin, Math.min(indianMax, Math.round(total*0.6)));
  let out = [...indianAll.slice(0, indianTarget), ...globalAll.slice(0, total - Math.min(indianAll.length, indianTarget))];
  ...
  if (curIndian > indianMax) {
    ...
    let removed = 0;
    out = out.sort((a,b)=>(a.score||0)-(b.score||0)).filter(e => {
      if (removed < globalsLeft.length && e.is_indian) { removed++; return false; }
      return true;
    });
...
function enforceBirthDeathCap(sel: EventItem[], pool: EventItem[], cap: number) {
  ...
  const toRemove = bd.sort((a,b)=>(a.score||0)-(b.score||0)).slice(0, bd.length - cap);
  ...
  const eventPool = pool.filter(e => e.kind === "event" && !selectedKeys.has(inKey(e))).sort((a,b)=>(b.score||0)-(a.score||0));
  for (const cand of eventPool) { if (trimmed.length >= sel.length) break; trimmed.push(cand); }
  return trimmed;
}

function enforceBattleCap(sel: EventItem[], pool: EventItem[], cap: number) {
  const isBattle = (x: EventItem) => /\b(battle|siege|crusade|skirmish)\b/i.test(x.title) || /\b(battle|siege|crusade|skirmish)\b/i.test(x.summary || "");
  const battles = sel.filter(isBattle);
  if (battles.length <= cap) return sel;
  const toRemove = battles.sort((a,b)=>(a.score||0)-(b.score||0)).slice(0, battles.length - cap);
  const rem = new Set(toRemove.map(e => e.title + "|" + e.year));
  let trimmed = sel.filter(e => !rem.has(e.title + "|" + e.year));
  const inKey = (e: EventItem) => e.title + "|" + e.year;
  const selectedKeys = new Set(trimmed.map(inKey));
  const nonBattlePool = pool.filter(e => e.kind === "event" && !selectedKeys.has(inKey(e)) && !isBattle(e)).sort((a,b)=>(b.score||0)-(a.score||0));
  for (const cand of nonBattlePool) { if (trimmed.length >= sel.length) break; trimmed.push(cand); }
  return trimmed;
}
...
export const app = new StateGraph<S>({ channels: {} })
  ...
  .addNode("verifyAndMerge", async (s) => {
    ...
    for (const e of (s.px ?? [] as PXItem[])) {
      ...
      if (!best || bestScore < 0.60) continue;
      const strict = /treaty|accord|agreement/i.test(best.title) || /treaty|accord|agreement/i.test(best.text||"");
      const gate = await requireDateConsensus(best.title || e.title, best.kind, mm!, dd!, strict);
      if (!gate.ok) continue;

      const yr = best.year ?? e.year ?? "";
      const date_iso = gate.iso ? gate.iso : (yr && /^\d+$/.test(yr) && Number(yr)>0 ? `${yr}-${mm}-${dd}` : null);
      const disp = yr && /^\-?\d+$/.test(yr) && Number(yr)>0
        ? `${readableDate}, ${yr}`
    ...
    const wikiDirect = (await Promise.all((s.wiki||[]).map(async (w) => {
      const strict = /treaty|accord|agreement/i.test(w.title) || /treaty|accord|agreement/i.test(w.text||"");
    ...
    for (const e of items) {
      const dup = out.find(o => (e.year && o.year && String(e.year)===String(o.year)) && jaccard(stripKnownPrefixAll(e.title), stripKnownPrefixAll(o.title)) > 0.72);
      if (!dup) out.push(e);
    }
  ...
  .addNode("selectEnforce", async (s) => {
    ...
    const minI = Math.round(total*0.60), maxI = Math.round(total*0.80);
  ...
  .addNode("enrich", async (s) => {
    ...
    s.events = await Promise.all((s.selected ?? [] as EventItem[]).map(async (e: EventItem) => {
      ...
      if (note && (e.summary||"").length < 240) e.summary = trimSummary(`${e.summary||""} ${note}`);
      return e;
    }));
    return s;
  })
